...
# ===============================
MODEL_NAME = "gpt-5.2"
client = OpenAI(api_key="")  # Uses env var if empty
...
# ===============================
# RAW METADATA (IMPROVED LABEL LOGIC)
# ===============================
raw_metadata = []
...
for col in df_raw.columns:
    ...
    # Get label from meta, fallback to cleaned-up column name
    label = label_dict.get(col, "")

    # If no label found, use a cleaned version of the column name
    if not label:
        label = col.replace("_", " ").title()

    # If no label or cleaned column name found, set fallback
    if not label:
        label = "Unnamed Column"
